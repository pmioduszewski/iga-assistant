...
struct FundamentalsView: View {
    ...
    /// Engine internals collapsed by default (fundamentals lead).
    @State private var showEngineDetails = false

    // MARK: Iga summary — plain language, what she IS + current state
    //
    // Replaces the wall of proactive-engine counters as the FIRST thing a
    // (possibly new) user reads. Derives the one-liner purely from the
    // already-computed health enum — no new state, no decision.
    private var igaSummary: some View {
        let status: String = {
            switch store.health {
            case .healthy:   return "Running quietly — all good."
            case .stale:     return "Catching up after a break — "
                + "she'll be current shortly."
            case .notRunYet: return "Idle — nothing to do yet."
            case .error:     return "Something needs a look "
                + "(open details below)."
            }
        }()
        return VStack(alignment: .leading, spacing: 6) {
            Text("Your personal assistant. She tracks your habits and "
                 + "coaches you on them, watches for things worth doing, "
                 + "and does quiet background work — all from the menu "
                 + "bar. Deleting this app changes nothing: /gm and the "
                 + "engine keep working on their own.")
                .font(.caption)
            ...
            Text("Habits live in the board → · right-click the menu-bar "
                 + "icon for settings.")
                .font(.caption2)
    ...
    var body: some View {
        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: sectionGap) {
                ...
                actions
                Divider()
                // Proactive-engine internals (counters, usage limits,
                // queue, last check, skills) are jargon for a new user —
                // collapsed by default behind one line. Fundamentals +
                // actions lead; the engine is a detail you can open.
                DisclosureGroup(isExpanded: $showEngineDetails) {
                    VStack(alignment: .leading, spacing: sectionGap) {
                        countsRow
                        Divider()
                        governorRow
                        ...
                        SkillsSectionView(
                            ...
                            discovered:
                                store.state.tick?.discoveredJobs ?? 0,
                            fired:
                                store.state.tick?.firedCandidates ?? 0)
                    }
                    .padding(.top, 6)
                } label: {
                    sectionHeader("Proactive engine · details")
