...
final class ContractLitmusTests: XCTestCase {
    ...
    private let widgetHostFiles = [
        ...
        // Mood widget — pure read-poll + pure render (strictly read-only;
        // logging is the engine's record entry point, never this view).
        "MoodWidgetStore.swift",
    ...
    func testGridIsReadOnlyCompactIsTheOnlyMutator() throws {
        // The Grid (dense) mode is now a READ-ONLY contribution
        ...
        // The shared settings cog (Compact + Grid) opens the manage sheet
        // only — it must never mark/relay, so Grid stays read-only for
        // marking while still exposing settings.
        let cog = members.first { $0.name == "settingsCog" }
        XCTAssertNotNil(cog)
        XCTAssertFalse(
            cog!.body.contains("relayToggle")
                || cog!.body.contains("runRecord"),
            "settingsCog must open the sheet only, never mark/relay")
    }
