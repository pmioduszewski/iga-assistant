...
def resolve(candidates, contradictory: bool = False):
    """Return (winner|None, reason). None winner => emit to needs_user."""
    if not candidates:
        return None, "needs_user: no candidates"
    ...
    if corrections:
        ...
        if contradictory and len(corrections) > 1 and \
           corrections[0].created_at == corrections[1].created_at:
            return None, "needs_user: contradictory same-day corrections"
        return corrections[0], "explicit correction overrides inferred"
    if contradictory:
        return None, "needs_user: contradictory non-correction facts"
