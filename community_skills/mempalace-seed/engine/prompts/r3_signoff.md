...
If unresolvable, add to `needs_user` in seed.final.
...
- If impossible, move the entry to `needs_user` for the user to provide source context.
...
```json
{
  ...
  "needs_user": [ /* unresolved items from all rounds */ ]
}
```
...
## Unresolved Items (needs_user)

<list of any items that require the user's clarification>

## Status

✅ Ready for publication if no items in needs_user.

❌ BLOCKED by needs_user items — the user must review and clarify before the seed can be considered complete.
```
...
- [ ] No internal contradictions remain (or all are documented in needs_user).
