...
If the tiebreaker can't decide (simultaneous corrections, incompatible facts with no date), emit to `needs_pablo` in seed.v2.
...
```json
{
  ...
  "categories": { /* updated */ },
  "needs_pablo": [ /* unresolved items carried forward */ ]
}
```
...
- [ ] Unresolved items are in `needs_pablo`.
