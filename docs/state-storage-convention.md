...
|---|---|---|---|
| Proactive ledger+governor | `~/Iga/state/proactive.db` **and** `~/Iga/state/proactive.db` | `iga-proactive/engine/ledger.py`,`governor.py` | `$IGA_PROACTIVE_DB` else **`~/Iga/state`** (code) — docstring says `~/Iga/state` |
...
app's `LedgerReader`, and every other store say **`~/Iga/state`**. Result:
...
`state.json` sat in `~/Iga/scratch`. The fix (deferred — migration not in
...
1. **One root: `$IGA_STATE_DIR`**, default `~/Iga/state/` — exactly the
   habit/mood substrate resolver (proven, isolation-aware, documented).
   Precedence `$IGA_STATE_DIR` > `$IGA_HOME/state` > `~/Iga/state`.
2. **Every persistent store lives under it**, namespaced by domain:
   ...
   - `state/<domain>.db` or `state/<domain>/` for any store a user adds with
     their own tooling (never the repo root)
