...
2. **Apply the tiebreaker** to resolve contradictions within a category:
   ...
   - If multiple explicit corrections share the exact same `created_at`, mark that conflict in `needs_user` — do NOT guess.
   - If no corrections exist and candidates are logically incompatible, emit to `needs_user` and skip.
...
5. **If genuinely unresolved** (contradictions you cannot resolve, missing critical context): append a note to `needs_user` explaining why.
